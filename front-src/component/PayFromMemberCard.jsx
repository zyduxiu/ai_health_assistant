...
import adjustBalance2 from "../Service/adjustBalance2";
   ...
   console.log(currentAppointment);
    ...
    const handlepay = async () => {
        try {
            ...
            console.log(balance);
            // 返回一个 Promise
            let k= await adjustBalance2(balance);
    ...
    const handleConfirm = async () => {
        ...
        try {
            // 使用 fetch 获取会员信息
            const response = await fetch(`${config.apiBaseUrl}/api/getMemberInfo?phoneNumber=${phoneNumber}`, {
                credentials: "include"
            });
            if (!response.ok) {
            ...
            if (!whetherpay) {
                ...
                // 如果已经加载会员信息，直接进行支付
                await handlepay();  // 执行支付操作
                setIsPaymentModalVisible(false);  // 关闭支付模态框
               // message.success("已成功支付");
            }
        } catch (error) {
