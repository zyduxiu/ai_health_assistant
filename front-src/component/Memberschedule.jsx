...
function Schedule({ date2 ,clientlist}) {
    ...
    const [cards,setCards]=useState(["无卡"]);
    ...
    function TimeSlot({ date,doctorName, time, appointment }) {
        ...
        const cancelAppointment =  () => {
            ...
            while (totalcost !== 0) {
                ...
                updatetimes.push(newitem);  // 不再直接使用 setCurrenttimes 频繁更新状态
            ...
            if (patient.attribute === "已预约" ) {
                return (
                    <Tag key={`${doctorName}-${time}-${index}`} color="pink" style={{
                        ...
                        marginLeft:'3%',
                        marginTop:'6px'
                    }} onClick={() => patient.name === members[0].name && handleClick2(index)}>{patient.name}</Tag>
                );
            }
            ...
            if (patient.attribute === "已诊") {
                return(
                    <Tag
                        ...
                        onClick={() => patient.name === members[0].name && handleClick2(index)}
                    >
                        {patient.name}
                    </Tag>
                );
                        ...
                        <Form>
                            ...
                            <div style={{
                                ...
                                {currentappointment && currentappointment.attribute === '已预约' && (
                                    <React.Fragment>
                                        <Button style={{marginLeft: '80%'}} onClick={cancelAppointment}>取消预约</Button>
                                    </React.Fragment>
                                )}
